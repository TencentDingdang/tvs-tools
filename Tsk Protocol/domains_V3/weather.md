...
1. 天气追问城市信息？
   ...
   目前天气获取当前位置的方式：1.gps定位（准确、覆盖率高）2.ip定位（只能精确到城市，可能查不出来）。因此如果没有上传gps或者ip定位不出来位置信息，都可能导致天气服务进行追问，因为城市信息是天气领域的”必选参数“，最多追问3次就会退出。
...
3. 如何知道问的是当天还是多天的天气？
   ...
   `iDayIndex`和`vFutureIndex`可以共同定位出来要查询的是当天还是多天的天气，具体的判断逻辑如下：首先，`iDayIndex>=0`表示的就是第几天的天气，`<0`就表示的是多天天气，此时就需要使用`vFutureIndex`数组字段中的值，里面存放了问询的那几天的天气。

   > iDayIndex和vFutureIndex存放的都是vcWeatherInfo数组的下标索引，比如0就代表的是当天天气，1代表的就是明天的天气，依次类推。